...
const createBooking = async (userId, payload) => {
    ...
    if (user.isDeleted || user.status !== "ACTIVE") {
        throw new AppError(status.FORBIDDEN, "Your account is not allowed to create a booking.");
    }
    ...
    if (timeSlot.room.isDeleted) {
        throw new AppError(status.NOT_FOUND, "The room for this time slot is no longer available.");
    }
    if (timeSlot.room.status !== "ACTIVE") {
        throw new AppError(status.BAD_REQUEST, "The room for this time slot is currently unavailable.");
    }
    if (payload.guestCount > timeSlot.room.capacity) {
        throw new AppError(status.BAD_REQUEST, `This room can accommodate a maximum of ${timeSlot.room.capacity} guests.`);
    }
    if (timeSlot.startTime <= new Date()) {
        throw new AppError(status.BAD_REQUEST, "This time slot has already started or passed.");
    }
...
const cancelBooking = async (userId, bookingId) => {
    ...
    if (booking.status === "COMPLETED") {
        throw new AppError(status.BAD_REQUEST, "Completed bookings cannot be cancelled.");
    }
    if (booking.payment && booking.payment.status === "PAID") {
        if (!booking.payment.paymentIntentId) {
            throw new AppError(status.INTERNAL_SERVER_ERROR, "Payment intent ID is missing.");
        }
...
const updateBookingStatus = async (bookingId, payload) => {
    ...
    if (!allowedStatuses.includes(payload.status)) {
        throw new AppError(status.BAD_REQUEST, `Cannot change booking status from ${booking.status} to ${payload.status}.`);
    }
    if (payload.status === "CANCELLED") {
        if (booking.payment?.status === "PAID") {
            ...
            if (refund.status !== "succeeded") {
                throw new AppError(status.BAD_REQUEST, "Payment refund failed. Booking was not cancelled.");
            }
            const result = await prisma.$transaction(async (transaction) => {
                const updatedPayment = await transaction.payment.update({
                    where: {
