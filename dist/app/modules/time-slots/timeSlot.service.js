...
const createTimeSlot = async (payload) => {
    ...
    if (existingTimeSlot) {
        throw new AppError(status.CONFLICT, "A time slot already exists for this room at this time.");
    }
    ...
    if (overlappingTimeSlot) {
        throw new AppError(status.CONFLICT, "This time slot overlaps with an existing time slot.");
    }
...
const getAllTimeSlots = async (query) => {
    const now = new Date();
...
const updateTimeSlot = async (timeSlotId, payload) => {
    ...
    if (existingTimeSlot.bookings.length > 0) {
        throw new AppError(status.BAD_REQUEST, "A booked time slot cannot be modified.");
    }
    ...
    if (overlappingTimeSlot) {
        throw new AppError(status.BAD_REQUEST, "This time slot overlaps with an existing time slot.");
    }
...
const deleteTimeSlot = async (timeSlotId) => {
    ...
    if (existingTimeSlot.bookings.length > 0) {
        throw new AppError(status.BAD_REQUEST, "A booked time slot cannot be deleted.");
    }
