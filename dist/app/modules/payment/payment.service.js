...
const createPaymentIntent = async (userId, bookingId) => {
    ...
    if (booking.status === "CANCELLED") {
        throw new AppError(status.BAD_REQUEST, "A cancelled booking cannot be paid for.");
    }
    if (booking.status === "COMPLETED") {
        throw new AppError(status.BAD_REQUEST, "This booking has already been completed.");
    }
    if (booking.status === "CONFIRMED") {
        throw new AppError(status.BAD_REQUEST, "This booking has already been confirmed.");
    }
    if (booking.expiresAt && booking.expiresAt <= new Date()) {
        throw new AppError(status.BAD_REQUEST, "This booking has expired. Please create a new booking.");
    }
    ...
    if (booking.payment?.paymentIntentId) {
        const existingPaymentIntent = await stripe.paymentIntents.retrieve(booking.payment.paymentIntentId);
        /*
        ...
        if (existingPaymentIntent.status === "succeeded") {
            throw new AppError(status.BAD_REQUEST, "This payment has already been completed.");
        }
        /*
...
const createCheckoutSession = async (userId, bookingId) => {
    ...
    if (booking.payment?.checkoutSessionId) {
        const existingSession = await stripe.checkout.sessions.retrieve(booking.payment.checkoutSessionId);
        /*
        ...
        if (existingSession.status === "complete") {
            throw new AppError(status.BAD_REQUEST, "This booking payment has already been completed.");
        }
        /*
